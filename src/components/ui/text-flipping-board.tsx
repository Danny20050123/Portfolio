...
const MIN_SCRAMBLE_STEPS = 5;
const MAX_SCRAMBLE_STEPS = 8;
const CLEAR_SCRAMBLE_STEPS = 3;

const BASE_TOTAL_S =
  ((BOARD_COLS - 1) * BASE_COL_DELAY +
    (BOARD_ROWS - 1) * BASE_ROW_DELAY +
    MAX_SCRAMBLE_STEPS * BASE_STEP_MS) /
  1000;
...
const ACCENT_COLORS: AccentColor[] = [
  ...
  { top: "bg-neutral-700", bottom: "bg-neutral-800", text: "text-white" },
  { top: "bg-stone-700", bottom: "bg-stone-800", text: "text-white" },
  { top: "bg-zinc-800", bottom: "bg-zinc-900", text: "text-white" },
];
...
const FlapCell = React.memo(function FlapCell({
  ...
  useEffect(() => {
    ...
    const scrambleCount =
      normalized === " "
        ? CLEAR_SCRAMBLE_STEPS
        : MIN_SCRAMBLE_STEPS +
          Math.floor(
            Math.random() *
              (MAX_SCRAMBLE_STEPS - MIN_SCRAMBLE_STEPS + 1),
          );
  ...
  const topBg = accent?.top ?? "bg-zinc-800";
  const bottomBg = accent?.bottom ?? "bg-zinc-900";
  ...
  return (
    <div className="flex aspect-3/6 flex-col overflow-hidden rounded-[2px] border border-zinc-800 bg-black shadow-[inset_0_1px_0_rgba(255,255,255,0.04)] md:rounded-[3px] md:border">
      {/* Flap content area */}
      <div className="relative flex-1 perspective-dramatic transform-3d">
        <div className="absolute inset-0 z-40 hidden flex-row items-center justify-center md:flex">
          <div className="h-1/2 w-px rounded-tr-sm rounded-br-sm bg-zinc-950" />
          <div className="flex h-px flex-1 bg-zinc-950" />
          <div className="h-1/2 w-px rounded-tl-sm rounded-bl-sm bg-zinc-950" />
        </div>
        ...
        >
          ...
          {flipId > 0 && (
            <motion.div
              key={`s${flipId}`}
              className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_bottom,rgba(255,255,255,0.16),transparent_65%)]"
              initial={{ opacity: 0.5 }}
              animate={{ opacity: 0 }}
        ...
        {flipId > 0 && (
          ...
          >
            ...
            <motion.div
              className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_bottom,rgba(255,255,255,0),rgba(0,0,0,0.9))]"
              initial={{ opacity: 0 }}
              animate={{ opacity: 0.6 }}
        ...
        {flipId > 0 && (
          ...
          >
            ...
            <motion.div
              className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_top,rgba(0,0,0,0),rgba(255,255,255,0.12))]"
              initial={{ opacity: 0.4 }}
              animate={{ opacity: 0 }}
              transition={{
                duration: flipDuration * 0.85,
      ...
      <div className="h-2 w-full bg-[repeating-linear-gradient(to_bottom,currentColor_0,currentColor_1px,transparent_1px,transparent_0.15rem)] mask-t-from-50% text-zinc-700 opacity-35 md:h-4 md:bg-[repeating-linear-gradient(to_bottom,currentColor_0,currentColor_1px,transparent_1px,transparent_0.2rem)]" />
    </div>
  );
},
(prevProps, nextProps) =>
...
const ColorCell = React.memo(function ColorCell({ color }: { color: string }) {
  return (
    <div
      className="aspect-3/5 rounded-[3px] border border-zinc-800 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)]"
...
export function TextFlippingBoard({
  ...
  return (
    <div
      className={cn(
        "relative mx-auto w-full max-w-3xl rounded-xl border border-zinc-800 bg-black p-2 shadow-[0_24px_80px_-24px_rgba(0,0,0,0.95)] md:rounded-2xl md:p-4",
        className,
      )}
    >
      <div
        className="grid gap-px rounded-lg bg-zinc-950 p-px md:gap-[3px] md:rounded-xl"
